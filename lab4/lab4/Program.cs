...
// Підключення залежних лабораторних(1-3)
using library;
...
public class Program
{
    public static int Main(string[] args)
    {
        ...
        // Для виведення вікна допомоги
        app.HelpOption();
        

        // Обробник команди `--version` - виводить автора
        app.Command("version", command => {

            command.Description = "Display the author and CLI version";

            command.OnExecute(()=>Console.WriteLine("CLI for Lab4 v1.0.0 created by R.Konstantyi"));

        });


        // Встановлення змінної для значень шляху до файлів input.txt + output.txt
            ...
            command.OnExecute(() => {
                ...
                if (File.Exists(inputOption))
                {
                    ...
                    if (string.IsNullOrEmpty(labValue)||(labValue != "lab1" && labValue != "lab2" && labValue != "lab3"))
                    {
                        ...
                        if (labValue == "lab1") Class1.Lab1Execute(inputOption, outputOption);

                        else if (labValue == "lab2") Class2.Lab2Execute(inputOption, outputOption);

                        else Class3.Lab3Execute(inputOption, outputOption);
